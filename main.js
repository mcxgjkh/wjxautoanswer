...
function createMainWindow() {
    mainWindow = new BrowserWindow({
        ...
        title: '问卷星自动答题器 V8.4.3 - By 满城箫管尽开花',
        show: false
    });
    ...
    mainWindow.once('ready-to-show', () => {
        mainWindow.show();
        // 强制设置窗口标题，覆盖页面标题
        mainWindow.setTitle('问卷星自动答题器 V8.4.3 - By 满城箫管尽开花');
...
function createWjxWindow(config) {
    ...
    console.log('V8.4.3 - 加载问卷页面（无痕模式）:', wjxUrl);
...
ipcMain.handle('clear-cookies', async () => {
    try {
        ...
        await session.defaultSession.clearCookies();
        return { success: true };
    } catch (error) {
        return { success: false, error: error.message };
    }
...
ipcMain.handle('import-banks-from-folder', async (event) => {
    try {
        ...
        const supportedFiles = files.filter(f => 
            f.toLowerCase().endsWith('.json') || f.toLowerCase().endsWith('.csv')
        );
        ...
        for (const fileName of supportedFiles) {
            ...
            try {
                ...
                if (ext === '.csv') {
                    // 使用 CSV 解析函数，传入文件名作为默认题库名
                    bankData = parseCSVToBank(fileContent, path.basename(fileName, '.csv'));
                } else {
                    // JSON 解析
                    bankData = JSON.parse(fileContent);
                }
                ...
                if (validateBankData(bankData)) {
                    ...
                    importedBanks.push({
                        error: `文件 ${fileName} 格式无效：缺少 name 或 answers`,
                        path: filePath
                    });
                }
            } catch (error) {
